...
2. **格律规则已内化进权重，不再依赖 prompt 引导。** `local_lora_naked`（只喂一句用户要求、不给任何格式约束）的平仄合格率 96.4%、押韵 39.0%，对比带完整格式 prompt 的 `local_lora`（95.4% / 32.9%）——**关键证据是拿掉格式约束后格律没有塌，而非数值上略高**（+1.0pp / +6.1pp 均在各自 std 内，不构成 naked 优于 full 的证据）。若格律靠 in-context 引导维持，移除引导应出现显著退化；实测未退化，指向规则来自权重本身。
3. **格律与评委偏好脱钩——这条改变了生产设计。** `local_base` 平仄合格率只有 25.6%（LoRA 的四分之一），pairwise 胜率反而是三个本地模型里最高的（0.356）。**⚠ 但不能据此推断「评委对格律权重极低」**：专项验证该推断的 F3 线已判定为**实验设计不可行**（需要「同一首诗只改平仄、不动意境」的对照，而在诗里做不到；非样本量问题），见 [`REPORT_F3_pingze_sensitivity_20260624.md`](REPORT_F3_pingze_sensitivity_20260624.md)。**成立且不依赖该推断的结论：生产中的格律保证必须留在 rule-based 硬门控里，不能交给 LLM judge**——规则可验证可复现，而评委判断与规则分的关系既未被证明、也无法被证明。
...
| 3. 格律差 4 倍而胜率不分 | 25.6% vs 95.4% | 3.0–6.4pp | 格律侧效应 ≫ 噪声；胜率侧本身即"无差异" |

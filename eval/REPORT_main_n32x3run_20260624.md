...
3. **评委对格律给的权重极低——这条是负面发现，且改变了生产设计。** `local_base` 平仄合格率只有 25.6%（LoRA 的四分之一），pairwise 胜率反而是三个本地模型里最高的（0.356）。据此推断 LLM 评委不靠格律决胜，专项验证见 [`REPORT_F3_pingze_sensitivity_20260624.md`](REPORT_F3_pingze_sensitivity_20260624.md)。**结论：生产中的格律保证必须留在 rule-based 硬门控里，不能交给 LLM judge。**

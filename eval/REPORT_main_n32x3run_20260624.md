# eval_poem 主跑报告 · LoRA 微调究竟改变了什么

_数据源：`outputs/eval/eval_poem_20260624_210002.json` · n=32（全部体裁 / 全部密度）× **3 个独立 run** · 评委 = deepseek-v4-pro + qwen-max + glm-4-plus + moonshot-v1-32k · 每模型每题 5 候选_

## 摘要（TL;DR）

1. **LoRA 提升地板，不提升天花板。** 候选合格率 pass@0.7 从 base 的 36.2% 升到 64.0%（≈ ×1.8），但 best 候选的 4 维 total 两者持平（0.771 vs 0.771）。LoRA 收紧了输出分布、砍掉了坏样本，没有抬高最好那一首的水平——alignment fine-tune 的典型形态。
2. **格律规则已内化进权重，不再依赖 prompt 引导。** `local_lora_naked`（只喂一句用户要求、不给任何格式约束）的平仄合格率 96.4%、押韵 39.0%，对比带完整格式 prompt 的 `local_lora`（95.4% / 32.9%）（均为 best 代表作口径，全候选口径同结论，见第 4 节）——**关键证据是拿掉格式约束后格律没有塌，而非数值上略高**（+1.0pp / +6.1pp 均在各自 std 内，不构成 naked 优于 full 的证据）。若格律靠 in-context 引导维持，移除引导应出现显著退化；实测未退化，指向规则来自权重本身。
3. **格律与评委偏好脱钩——这条改变了生产设计。** `local_base` 平仄合格率只有 25.6%（LoRA 的四分之一，best 代表作口径；全候选口径 22.2% vs 94.3%，同结论，见第 4 节。**「四分之一」这个倍数依赖 ≥0.8 阈值，换 ≥0.75 则为 1/1.5，见第 4 节阈值敏感性**），pairwise 胜率反而是三个本地模型里最高的（0.356）。**⚠ 但不能据此推断「评委对格律权重极低」**：专项验证该推断的 F3 线已判定为**实验设计不可行**（需要「同一首诗只改平仄、不动意境」的对照，而在诗里做不到；非样本量问题），见 [`REPORT_F3_pingze_sensitivity_20260624.md`](REPORT_F3_pingze_sensitivity_20260624.md)。**成立且不依赖该推断的结论：生产中的格律保证必须留在 rule-based 硬门控里，不能交给 LLM judge**——规则可验证可复现，而评委判断与规则分的关系既未被证明、也无法被证明。

**结论可复现性：** 三条结论依赖的效应幅度与噪声地板对照如下——

| 结论 | 效应幅度 | 3 run std | 判读 |
|---|---|---|---|
| 1. 合格率提升 | +27.8pp | 0.024–0.026 | 效应 ≫ 噪声，**稳固** |
| 2. 格律未因移除 prompt 而退化 | +1.0pp / +6.1pp | 3.0–6.2pp | 效应 ≈ 噪声，**只支持"未退化"，不支持"更优"** |
| 3. 格律差 4 倍而胜率不分 | 25.6% vs 95.4%（best 口径；全候选 22.2% vs 94.3%。**倍数对阈值敏感**：≥0.75 下为 1.5×，见 §2.4） | 3.0–6.4pp | 格律侧效应 ≫ 噪声；胜率侧本身即"无差异" |

pairwise 胜率是全表波动最大的指标（`local_base` std 0.044，逐 run 0.303–0.411），但三条结论均不依赖胜率的绝对数值。

> `qwen-plus` 在本表中是**参照基线**而非竞争对象——它胜率 0.997、4 维 total 0.909，与 1.5B 本地模型不在一个量级。它的作用是给"评委判断是否有区分能力"提供一个已知答案的锚点：如果评委连 qwen-plus 和 base 都分不出来，那本报告的其余数字都不必看。

---

## §0 这个实验要回答什么

一句话：**判断 LoRA 微调是否真的让 1.5B 本地模型写诗写得更好，以及"更好"具体好在哪一层。**

被比较的四个对象：

| 模型 | 说明 | prompt 模式 |
|---|---|---|
| `local_base` | Qwen2.5-1.5B 原始权重，未微调 | full |
| `local_lora` | Qwen2.5-1.5B + 古典诗词 LoRA | full |
| `local_lora_naked` | 同上权重，但**不给格式约束 prompt** | naked |
| `qwen-plus` | 百炼 API 大模型，参照基线 | full |

`full` = 与 API 相同的 system + 格式约束 + 用户要求（受控对照）；`naked` = 仅一句简短用户要求。设 naked 臂的目的是做 **ablation**：如果拿掉格式 prompt 后格律不塌，说明规则已经进了权重，而不是靠 in-context 引导临时撑起来的。

## §1 方法：在没有标准答案的领域怎么比较两个模型

诗歌质量没有客观真值，可用的裁判只有同样不可靠的 LLM。所以本实验的方法设计要先处理三个已知障碍，每个障碍对应一个设计决定：

**障碍一：LLM 打绝对分会饱和。** 同一首诗两次调用的分数能差 1 分，五个候选的分数全簇集在 7.5–9.0 区间，区分度接近于零。

→ **对策：全流程只让评委做选择和比较，不让它打分。** 具体分两步：

- **BWS 选 best**：BWS 即 **Best-Worst Scaling（最优-最劣标度法）**，源自心理测量学的偏好测量方法——不要求受试者打分，只要求在一组选项里指出最好（和最差）的那个，以此回避"不同人对 7 分的理解不同"这类量表使用偏差。*本实验只用了 Best 一半*：把 5 个候选一次性给评委，让它返回最佳的那一首的编号，不问最差、也不做效用尺度估计，严格讲属于 best-only forced choice。选出的这首即该模型在该题上的**代表作**。
- **跨模型 pairwise**：四个模型的代表作两两对决，评委只回答"A 和 B 哪首更符合用户要求"。

  > **为什么代表作必须由评委选、不能用本地规则分选**：本地规则分里平仄占 0.25、押韵占 0.15，格律合计 40% 权重，而 LoRA 的格律合格率是 base 的近 4 倍。若用规则分挑代表作，等于先用一把明显有利于 LoRA 的尺子选出 LoRA 的最佳候选，再宣布 LoRA 获胜——循环论证。让评委自己选代表作，才能保证"选拔标准"与"对决标准"是同一套。

**障碍二：单个评委不可信，且模型会偏爱自己家族的输出（self-bias）。**

→ **对策：4 个跨家族评委独立判定。** DeepSeek + Qwen + GLM + Moonshot 分属四个不同厂商与训练体系，任一家族的 self-bias 无法主导结果。BWS 环节 4 票多数决（平票落回本地分兜底）；4 维评分环节取中位数（≥3 个有效值时）以抗异常值；任一评委解析失败则**显式弃权**、不计入合成，避免把失败静默当成某一票。

**障碍三：评委存在位置偏置——可能只是因为某首诗排在前面就选它。**

→ **对策：每一对都判两次（forward + reverse），交换 A/B 位置。** 两次判定方向一致才计为有效票；两次都选同一个位置（永远选 A 或永远选 B）说明它跟着座位走，记为**摇摆**、不计入胜负。摇摆率因此成为一项公开指标，本主跑口径下约 23%。BWS 环节同样在每次调用前随机打散候选顺序。

**障碍四：单次运行分不清"真实差异"和"LLM 噪声"。**

→ **对策：完整跑 3 个独立 run。** 每个 run 从生成候选开始重跑整条 pipeline，temperature 噪声会产生不同的候选和不同的评委判断。报告中所有指标以 `mean ± std` 呈现，**std 即噪声地板**——效应幅度小于 std 的差异一律不作判读。

**完整流程：**

```
每模型生成 5 候选
   ↓  4 评委各自 BWS 五选一 → 多数决 → 该模型的代表作(best)
代表作跑 multi-judge 4 维评分（4 评委中位数合成）
   ↓
四模型代表作 round-robin 跨模型 pairwise（每对 × 每评委 × forward/reverse 双向）
   ↓
以上整条链路独立重复 3 次 → mean ± std
```

## §2 结果

> 单 run 的全部候选诗、BWS 逐票明细与 pairwise 详情见本报告后半（基于 Run 1，由 `eval_poem.py` 自动生成）。
...
**判读：** 三个本地模型的胜率都在 0.31–0.36 之间，彼此差距均在 1 个 std 内，**评委分不出 base 和 LoRA 谁的代表作更好**。但注意 `local_base` 数值上最高，而它的平仄合格率只有 25.6%（见第 4 节）——这个矛盾是摘要第 3 条的来源。`qwen-plus` 0.997 的胜率确认了评委在量级差距明显时具备区分能力。
...
**判读：** 本节是摘要第 1 条的证据。`pass@0.7` 候选合格率 0.362 → 0.640（+27.8pp，≈ ×1.8）是全报告最大的效应，且 3 run std 仅 0.024–0.026，稳固。同时 `min` 候选分（worst-case）0.600 → 0.660 上抬——**LoRA 的收益集中在"少出烂诗"，而不是"多出好诗"**。
...
**判读：** best 候选的 `total` 三个本地模型持平（0.771 / 0.771 / 0.776），佐证"天花板未被抬高"。反常之处在 `intent` 与 `imagery`：`local_base` 反而最高（0.939 / 0.912）。合理解释是 base 不受格律约束、遣词更自由，容易写出评委偏好的意象密度；而 LoRA 为满足格律牺牲了一部分表达自由度——这是微调的代价，与摘要第 3 条互为印证。

### 4. 格律合规（mean ± std across runs）

> **样本口径：本表的分母是 BWS 代表作，不是全部候选。** 每题 5 候选先经 4 评委投票选出 1 首代表作，合格率只在这 32 首/run（3 run 共 96 首）上统计，而非 160 首/run 的全候选（实现见 `eval_poem.py` `_aggregate()` 的 `pingze_pass@0.8`，取自 `best_scores`）。分母与第 2 节的 `pass@0.7`（全候选口径）不同，两表不可直接互推。
> `pingze` / `rhyme` 本身是 `core/poem/scorer.py` 的 rule-based 分（`_score_pingze` / `_score_rhyme`），确定性、不经 LLM，run 间波动只来自候选与代表作选择的变化。
...
**判读：** 本节同时是摘要第 2、3 条的证据。**第 2 条**：`local_lora_naked` 拿掉格式 prompt 后平仄 96.4%、押韵 39.0%，对比 full 模式的 95.4% / 32.9% —— **两项均未退化**（数值上的略高落在各自 std 内，不可读作 naked 更优），说明格律来自权重而非 prompt。**第 3 条**：base 平仄 25.6% vs LoRA 95.4%，差距近 4 倍，而两者 pairwise 胜率（第 1 节）却在噪声范围内难分——评委没有把这个巨大的格律差距计入判断。

**全候选口径稳健性（2026-08-11 补算）：** 上述口径的一个自然疑问是"代表作是被挑过的，比率会不会被筛选抬高"。原始 artifact 保留了每个候选的 rule 分（`candidates[].local_scores`），据此重算 160 首/run 全候选口径如下：

| 平仄合格率 (≥0.8) | `local_base` | `local_lora` | `local_lora_naked` | `qwen-plus` |
| --- | --- | --- | --- | --- |
| 全候选（160/run） | 22.2% ± 3.5% | 94.3% ± 0.8% | 92.7% ± 2.5% | 88.1% ± 2.2% |
| best 代表作（32/run，上表） | 25.6% ± 6.4% | 95.4% ± 4.3% | 96.4% ± 3.0% | 91.1% ± 1.4% |

筛选带来的偏移为 +1.1 ~ +3.7pp，**四个模型全部落在各自 std 内，第 2、3 条结论在两种口径下一致**。偏移之所以这么小，是 §1 那条反循环论证设计的直接后果：代表作由**评委**选、不由规则分选，而评委的判断与格律基本脱钩，因此筛选环节几乎不携带格律倾向——这反过来又是摘要第 3 条的一个独立旁证（注意它与第 3 条同源，不构成对第 3 条的独立验证，仍受 §3 局限 2 约束）。全候选口径 std 更小（如 `local_lora` 0.8% vs 4.3%）仅因样本量 ×5。

**⚠ 阈值敏感性：「4 倍」这个倍数是阈值选择的产物（2026-08-11 补算）。** `pingze` 分 = 合规句数/总句数，取值离散——4 句诗只能取 0 / 0.25 / 0.5 / 0.75 / 1.0，8 句诗取 k/8。**阈值 0.8 不落在任何可取值上**：对绝句而言 ≥0.8 等价于「必须 = 1.0，四句零出律」，对律诗是「≥7/8」。而 `local_base` 的分布众数恰好卡在 0.75（其 best 代表作 40% 取值为 0.75），正好被这道门槛整个切掉。阈值挪一格，两侧变化极不对称（best 代表作口径，与上方主表同源）：

| 平仄合格率 | `local_base` | `local_lora` | LoRA / base |
| --- | --- | --- | --- |
| ≥0.75（绝句 3/4 句合律） | 65.6% | 97.7% | 1.5× |
| **≥0.8（本报告口径）** | **25.6%** | **95.4%** | **3.7×** |
| =1.0（完全合律） | 20.1% | 88.6% | 4.4× |

**定性结论不受影响**——任何阈值下 LoRA 的格律都远好于 base，摘要第 3 条依赖的是「格律差距大而胜率不分」，这在三种阈值下都成立。**但「近 4 倍」「LoRA 的四分之一」这类表述对阈值敏感**，脱离 `≥0.8` 这个口径复述会失真（同一份数据在 ≥0.75 下只有 1.5 倍）。跨报告引用时应连同阈值给出，或改用不依赖倍数的表述。`rhyme` 同理且更极端：其取值仅 {0.3, 0.6, 1.0}，≥0.8 等价于「偶数行全部同一韵部」。阈值语义详见 [`METHODOLOGY.md`](METHODOLOGY.md) §10.2。

> 押韵合格率三个本地模型均低于 40%，是本轮全部指标里最弱的一项，也是 LoRA 训练的已知短板，独立于本报告的三条结论。
...
**判读：** 逐 run 展开是为了让读者自己看噪声幅度。`local_base` 胜率在 0.303–0.411 之间摆动（跨度 0.108），是全表最不稳的一项——这正是不能只跑一次的理由；相比之下 avg 候选分和 pingze 的 run 间波动小得多，说明**生成侧指标比评委侧指标稳定**。

## §3 局限与边界

本报告的结论有以下几处不能外推，如实列出。**第 1、4 条为 2026-07 后续工作的回溯补注**，主跑执行时（2026-06-24）尚不知情：

1. **摇摆率 23% 是旧口径。** 该数值把「评委 API 调用失败」与「真实 position bias」混在同一个"摇摆"桶里。2026-07-05 起 `compare_poems` 引入显式弃权语义，两者已分桶，新口径与本报告数值不严格可比（跨期引用须注明）。同期无混杂探针给出的摇摆率为 21.9%，数值接近，提示旧桶的主要成分确为真实 position bias 而非 API 失败——但这是旁证，非定论。
2. **摘要第 3 条（评委轻视格律）是初步推测，不是定论。** 本报告提供的是间接证据（格律差 4 倍而胜率不分）；专项验证报告的 controlled pair 仅 n=4，且受 BWS 筛选影响——极端格律样本可能在选代表作时已被筛掉，使可用样本偏小。**强结论需要独立的 64-pair 重跑，尚未执行。**
3. **押韵合格率三个本地模型均 < 40%**，是训练侧的已知短板，本报告不对其做归因。
4. **本报告是评估侧口径，与生产路径存在已知差异**：评估用 4 评委 + 6 维（含 aesthetics）+ 中位数合成，生产用单评委 + 5 维；评估的 pairwise 双向判定，生产为单向 + A/B 位随机化。差异明细见 [`METHODOLOGY.md`](METHODOLOGY.md) §11。**因此本报告的数字描述的是模型能力，不是用户实际拿到的那首诗的质量。**

---

# Run 1 详细报告（候选 dump / pairwise 详情）

> 以下为 `eval_poem.py` 自动生成的单 run 完整产物，含 32 道题的全部候选诗、每位评委的 BWS 逐票明细与 pairwise 对决详情，供追溯核查。上文 §2 的聚合数字即由 3 份此类产物汇总而来。**自动生成区，请勿手工编辑。**

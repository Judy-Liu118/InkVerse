...
⚠ 按当前分析口径跑没有意义（2026-08-10 标注，见 REPORT_F3 §4）
────────────────────────────────────────────────────────────────
本脚本原本的目标是"把 n=4 的 anecdote 推到 n≥64 拿显著结论"。核实后该
路径不成立：**问题不在样本量，在判据**。筛出"base 出律但意境 ≥ lora"再
看 base 胜率，无法区分两种相反解释——(A) 评委不看格律；(B) 评委看格律，
但意境优势足够大压过了它。n 从 4 涨到 64 只是把同一个无法区分的量测得
更准。

改成剂量-反应设计才有价值
────────────────────────────────────────────────────────────────
正确形态：按**意境优势幅度分层**，看 base 胜率随意境优势的变化曲线，取
**意境优势趋近 0 时的截距**——那才是能回答 F3 的量。
  · 截距 ≈ 50%  → 意境持平时格律不影响胜负，格律权重确实低
  · 截距明显 <50% → 意境持平时格律占优方仍胜，格律权重不低
所以收池时应**覆盖意境差的整个范围**（--intent-margin 保持 0 或更低），
而不是只收意境优势大的样本；分析端必须分层出曲线，不能报一个总胜率。

即使改成剂量-反应设计，障碍仍在
────────────────────────────────────────────────────────────────
"格律与意境在诗中不可正交"这道坎不会因为换设计而消失：改平仄必然改字，
改字必然改意境，拿不到"同一首诗只改平仄"的对照。剂量-反应设计能得到的
是**相关性的形状，不是因果**。改完设计不等于问题已解决——这一句务必保留。

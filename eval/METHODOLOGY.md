...
> 📌 **摇摆率旁证（2026-07-06 B1b 无偏探针）**：新口径（弃权已分桶，本次 abstain=0）下，32 对新鲜配对（armA/armB + delta 两档终诗，与 judge 判定无关）双向重判实测摇摆率 **21.9%**，与历史混杂口径的 23% 数值接近——提示历史"摇摆"桶的主要成分是真实 position bias 而非 API 失败。方向上 7 例位置追随全偏 A 位（p=0.0156，primacy）。注意可比性是提示性的：B1b 为单 judge（qwen-plus）、诗对分布也不同于 n=32 主跑的跨模型对局；追随样本仅 7 例，作旁证不作定论。数据：`outputs/eval/probe_position_bias_fresh_20260706_082356.json` · 脚本 `eval/_probe_position_bias_fresh_pairs.py` · 详见 sweep 报告 §6.3 #1。

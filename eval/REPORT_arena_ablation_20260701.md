...
> **总纲 · 这个消融实验测不出擂台的价值，因为尺子和东西不匹配。**
>
> §2.10 逐首通读 18 组攻擂前/后诗后可以确认，**擂台实际在做的**是：修病句、修季节/时刻矛盾、去重字、把抽象词换成具象词、提升辞采。而**本实验测的**是：用户点名的物体有没有画进图里。**两者正交**——所以测不出价值不等于没有价值，是拿体温计量了身高。
>
> 更要紧的是：两个目标在当前接线下**不只是无关，而是互相拆台**。§2.10 的 4 例改坏无一例外都是「优化辞采时顺手扔掉用户的具体要求」。这就是结论④红框那条改动的全部意义。
...
- **保留理由**：擂台的目标函数是诗质量（辞采、意境），本实验从未测过它。§2.10 逐首审读 18 组攻擂前/后诗：**7 例修掉了可核查硬伤**（季节矛盾、时刻矛盾、句内重字、病句）、6 例文辞提升、1 例持平、**4 例改坏**；全篇重字 15 → 8，句内真重字 2 → 0。**擂台确实在做实事**（判读局限见 §2.10 开头）
- **⚠ 但代价是结构性的**：§2.10 的 4 例改坏**全部**是「丢掉用户要的东西」（21 丢古刹钟鼓、17 丢荒台、7 溪声偏题、14 客愁跑成闺怨）——不是偶尔失手，是三个改诗环节都拿不到 `user_request` 的必然后果
...
| §2 | 四条核心证据（CLIP × 2 + 硬约束 × 2），主池 + 辅池并列；§2.5 显著性分析（2026-07-05 补）· §2.6 跨家族 VLM 复核 · §2.7 kw 定义敏感度全池扫描（2026-07-06 补）· §2.8 诗侧闸门分解（2026-08-29 补）· **§2.9 攻擂前/后配对复现（2026-08-31 补，新代码上净 −5）· §2.10 人工文本审读：擂台到底改了什么（2026-08-31 补）** |
...
**擂台把冬天改成了秋天。** 这条与 §2.8 中「擂台修掉了 0702 run 里『灯明三伏夜』这处季节硬伤」的观察正好相反——**同一机制一次修好季节、一次改坏季节**。故不能把「修季节矛盾」记为擂台的系统性能力，它是无方向改写的两种随机结果。详见 §2.10。
...
## §2.10 人工文本审读：擂台到底改了什么（2026-08-31 补，零 API）

> **动机**：§2.8/§2.9 用「用户点名的物体在不在诗里」这一个二值指标衡量擂台，结论是净效应 ±0 与 −5。但这把尺子**只覆盖擂台工作的一小部分**——它对辞采、句法、逻辑一律看不见。本节逐首通读 0702 run 的 18 组「攻擂前/后」诗，回答：**擂台实际在改什么？**

### 方法与局限（先说清楚，结论按此打折）

- **样本**：0702 run 中诗被实际改动的 18 题（另 4 题攻擂 0 次，v0 ≡ 终稿，不计）。另参照 §2.9 的 0706 run 与 22 组 armA/armB 对照。
- **判读人**：Claude（Opus 5）。**外生性成立**——不同模型家族、不同提示词，且**完全没参与过 arm B 的筛选**，循环性断在这里。
- **⚠ 三条局限**：① **非盲评**，判读时知道哪首是攻擂后，期望效应无法排除；② **LLM 之间可能有相关误差**（同样偏好对仗工整、意象密度高、避免直白说理），与 `qwen-plus` 意见一致**不等于**都对；③ 分类含主观成分。**故下表把「修掉可核查硬伤」（客观、可复核）与「文辞提升」（主观）分开计。**

### 分类判读（18 题）

| 类别 | 题数 | 题号 |
|---|:--:|---|
| **A · 修掉了可核查硬伤** | **7** | 4 飞雪寒鸦、5 雪夜、6 炉火寒灯、8 高楼远山、12 征戍、16 长亭折柳、18 吊古 |
| B · 文辞提升但无硬伤变化（主观） | 6 | 1 疏桐寒蛩、10 归隐、11 戍楼角声、15 送别、20 重阳、22 无常 |
| **C · 改坏** | **4** | 7 溪声、14 客愁、17 古城荒台、21 古刹钟鼓 |
| D · 基本持平 | 1 | 19 中秋 |

（此表修正了初步口头判读的「7 好 / 3 坏 / 8 平」——重读后 A 类不变，B 类从「持平」上调，C 类多出 14 客愁一例。）

**A 类的证据是可核查的**，各举一处：

| 题 | 攻擂前的硬伤 | 攻擂后 |
|---|---|---|
| 5 雪夜 | 「灯明**三伏**夜」——**雪夜写三伏，季节硬伤** | 「峰峦吞夜黑／云气压窗冥」 |
| 8 高楼远山 | 「孤城**落照晓莺**眠」——**黄昏配清晨，时刻矛盾** | 「孤城衔暮晚钟悬」，且「晚钟」可画 |
| 6 炉火寒灯 | 「老伴茶**香**一夜**香**」——**句内重字**；「邻家**酒价**」抽象不可画；「暗销半枕」不通 | 「老伴茶瓯带影香」「邻家**酒旆**因风寂」「暗移半榻」 |
| 16 长亭折柳 | 「**落**叶千林**落**日间」——**句内重字**（另有夜/长两处全篇重字） | 「空山万木带烟残」 |
| 4 飞雪寒鸦 | 「飞入寒鸦一片时」近乎病句；冬景题写「万叶垂」且**通篇无雪** | 「雪压虬枝犹抱月」 |
| 12 征戍 | 「不为**霖**」「又成**霖**」——**全篇重字**，且春雨与征戍不搭 | 「铁甲凝寒犹望北／半蓑烟雨自成霖」 |
| 18 吊古 | 「衣锦谁怜未少多」不成话 | 「关塞云崩龙气在／河山血淬骨锋坚」 |

### 关键发现：擂台改坏的时候，**坏在同一个地方**

C 类 4 例，无一例外都是**丢掉了用户要的东西**：

| 题 | 丢了什么 | 表现 |
|---|---|---|
| 21 古刹钟鼓 | **两个具名物体全丢** | 「**古刹**钟声…」→「空山鹤影掠松桠／…半窗斜照落袈裟」——文字更精致，庙和钟都没了 |
| 17 古城荒台 | **荒台**，且新引入重字 | 保住「废堞」丢掉台；末联「青**磷**起／寒**磷**照」**全篇重字为擂台新引入** |
| 7 溪声 | **切题** | v0 有「潺湲」「水声」；终稿「松影摇溪浅／苔痕浸水闲」全是视觉，**诗题就叫溪声，声音消失** |
| 14 客愁 | **切题** | v0 写天涯／客梦／惜春愁，切题；终稿「罗袖薄」「玉钗柔」跑成闺怨，且「春事老」与「故园秋」**春秋并置矛盾** |

**擂台的失败模式是单一且可预测的**：它优化辞采时，会顺手扔掉用户的具体要求。这与 §0 结论④（三个改诗环节都拿不到 `user_request`）完全吻合——**它不是偶尔失手，是结构上没有守住这些东西的能力**。
...
| | 攻擂前 | 攻擂后 |
|---|:--:|:--:|
| 全篇重字（一首诗内某字 ≥2 次） | **15** | **8** |
| 句内重字 | 2 | 2 |

句内那栏看似不变，**但性质相反**：攻擂前的 2 处（「茶香一夜香」「落叶千林落日间」）**是真病**；攻擂后的 2 处（「霜骨**棱棱**」「客路**迢迢**」）**是合法叠字**。按「病」计为 **2 → 0**。

**但有反例**：17 古城荒台的终稿新引入「废」「磷」两处全篇重字，是全场最差的一首。

**⚠ 归因保留**：守擂 judge 的 rubric 只有主题契合／意象意境／语言质量／格律四维，**一个字没提重字**。所以准确说法是「**改完之后重字少了**」，不是「**擂台在修重字**」——很可能是用词转雅带来的副产品。

### 「修季节矛盾」不能记为擂台的能力

0702 run 里擂台修掉了 5 雪夜的「灯明三伏夜」；但 §2.9 的 0706 run 里，它把冬景七律从「**雪消**江路已平畴」改成「霜凋木落正**清秋**」——**把冬天改成了秋天**。

**同一机制一次修好、一次改坏。它不是在修季节，是在无方向改写，改到哪算哪。**
...
| | |
|---|---|
| **擂台实际在做的** | 修病句、修季节/时刻矛盾、去重字、把抽象词换成具象词、提升辞采 |
| **消融实验测的** | 用户点名的物体有没有画进图里 |

**两者正交。** 所以这个消融实验测不出擂台的价值，**不是因为擂台没价值，而是因为拿体温计量了身高**。

同时这也解释了为什么 §2.9 会测到 **−5**：擂台在优化它的目标（辞采）时，会**主动牺牲**测量指标关心的东西（具名物体）——两个目标在当前接线下是**互相拆台**的，而不是无关的。这正是 §0 结论④红框那条改动的意义所在。

### 由此得出的指标提案：可核查硬伤率

四类缺陷，**全部规则可判、不需要审美、且外生于擂台**（守擂 rubric 一条都没有）：

| 优先级 | 类别 | 可自动化程度 |
|---|---|---|
| 1 | **季节 / 时刻矛盾**（冬景题出现「秋」「三伏」；同诗内「落照」与「晓」并置） | 词表 + 规则，中等 |
| 2 | **病句 / 不成话**（需 LLM 判，但可用外生模型） | 低 |
| 3 | **偏题**（诗题关键词在诗中无对应） | 高，同 `POEM_TOKENS` 思路 |
| 4 | **重字**（区分叠字与真重字） | **高，纯字频统计** |

第 4 类本节已实现（15→8 / 2→0）。**建议下一步先把第 1、3 类规则化**——它们是本节手工发现的两个最典型失败模式，且都不需要 API。

- 数据：`outputs/eval/sweep_pairwise_win_delta_20260702_105404.json` 的 `poem_evolution[0].poem` vs `poem`（18 题有改动）；参照 `outputs/eval/rerun_arena_randomized_main_n22_20260706_224424.json`（§2.9）与 `outputs/eval/probe_position_bias_fresh_20260706_082356.json`（armA/armB 22 组对照）。重字统计为逐首 `Counter` 字频，全过程零 API。

...
════════════════ run 2 预登记（2026-07-10，跑前 commit，不得事后更改）════════════════
目的：run 1（eval/runs/llm_loop_ab，n=27，REPORT_llm_loop_ab_n27_20260710.md）为
single-shot，观测效应量（循环子集 mean Δ=+0.004）与 CLIP 单次评分噪声同量级。
run 2 以完全相同的预登记条款（题集/顺序/参数/指标/判读口径全部沿用，零改动）
独立重复一次，检验结论的 run-to-run 稳定性：
  · 跑法：--run-dir eval/runs/llm_loop_ab_run2（预算即真实剩余 gen 50 / edit 57，
    以 --gen-budget 50 --edit-budget 57 显式声明，题边界截断规则原样生效）；
  · 判读预定死：两次运行的"方向 + 效率结论"一致 → 报告为稳定复现；不一致 →
    如实报告为噪声主导，不挑更好看的一次；run 1 与 run 2 不合并混算主表
    （合并仅允许出现在明确标注"两次运行合并"的附注中）；
  · run 2 期间不修改任何 core/eval 被测代码。
═══════════════════════════════════════════════════════════════════════════════════
"""

...
> 生产 dual (α=0.6) 与 VLM oracle 的 Spearman ρ = **+0.365**，点估计高于 prompt_only (+0.301)、稍低于 poem_only (+0.384)——注意 n=15 时 Spearman ρ 的标准误约 0.29，这些差值均**未达统计显著**，只是方向一致的排序信号；α grid search 表明最优 α=0.8 处 ρ = **+0.414**，超过任一单 anchor 上限。**dual 设计在千古名诗语料上被数据支持；生产权重 α=0.6 不是 grid 上的最优点，但与最优点的 0.049 差距远小于噪声量级，不构成调参依据（详见 §4）**。均值层 mean(dual) 与 mean(single) 差距 ≤ 0.003（配对胜率 53.3%）——**信号在排序相关性维度、不在数值均值维度**。
...
- 生产 α = 0.6 的 ρ = +0.365，低于最优点 0.049 —— 但**这个差值不构成调参依据**：α ≥ 0.5 区间内各格点 ρ 的起伏只有 0.01 ~ 0.05 量级，远小于 n=15 时约 0.289 的标准误；且曲线在该区间**非单调**（0.5 → 0.6 反降 0.019、0.9 → 1.0 再降 0.008），是噪声主导的形态。**在这个样本量下，α ≥ 0.5 的各个取值之间谁高谁低读不出来**

### 工程建议

**⚠ 不建议直接改 `config.py:CLIP_POEM_WEIGHT` 到 0.8**，原因：

1. **最根本的一条：0.6 与 0.8 的差距读不出来**。两者 ρ 差 0.049，而 n=15 时 Spearman 标准误约 0.289；「α=0.8 最优」是在 11 个格点上取 max 的点估计，grid 曲线本身非单调。换言之，现有数据不足以支持「0.8 比 0.6 好」这个判断，调参会是在噪声上做决策
2. n=15 全 rich 数据，泛化到 sparse benchmark 未验证
3. 数据源单一 backend（qwen-image-2.0-pro），跨 backend 未验证（**§4.1 已补测：最优 α 不跨 backend 稳定**）
4. 千古名诗数据分布特殊（详见 §8 caveat）

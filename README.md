...
flowchart TD
    ...
    subgraph P ["诗 —— 本地 LoRA + 两级擂台"]
        ...
        D --> E{"合格 ≥ 3 首?"}
        E -- "否 · 重新生成 ≤3 轮" --> C
        E -- "是" --> F["五维评分<br>本地 平仄 · 押韵 · 意象 · 连贯 + LLM 切题<br>Top3 轮循 pairwise → 冠军"]
...
_合格率为 best 代表作口径（每题 5 候选经评委选出 1 首，n=32/run），非全候选；全候选口径同结论。_
...
_合格率为 best 代表作口径（n=32/run）；全候选口径（160/run）为 base 22.2% / LoRA 94.3%，同结论。_
